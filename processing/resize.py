...
def resize(orig, factor, method="nearest"):
    ...
    :param method: string, interpolation method to use when resizing. 'nearest' by default
    ...
    try:
        return zoom(orig, factor, order=method_dict[method])
    except RuntimeError:
...
def resize_to(orig, out_size, method="nearest"):
    ...
    :param orig: n-dimen numpy array, array to resize
    ...
    try:
        if len(orig.shape) != len(out_size):
            raise ValueError("Factor sequence length does not match input length")
        factors = tuple(0 if in_length == 0 else out_length / in_length
                        for out_length, in_length in zip(out_size, orig.shape))
    except TypeError:
        # thrown in out_size is not an iterable or doesn't contain numbers
