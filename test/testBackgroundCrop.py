...
class BackgroundCropTestGroup(unittest.TestCase):
    ...
    def test_cropOutputs(self):
        ...
        self.assertEqual(crop(testImage).shape, (3, 3, 3, 3), "Cropped output is in expected shape")
        ...
        self.assertEqual(crop(testImage, 0, (-2, -1)).shape, (3, 10, 3, 3), "Crop only specified axes")
        self.assertEqual(crop(testImage, 0, tuple()).shape, (3, 10, 10, 10), "Crop skipped when no axis specified")

        self.assertEqual(crop(testImage, 0, padding=1).shape, (3, 5, 5, 5), "Crops to expected shape with padding")
